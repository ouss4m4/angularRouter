...
export class ServiceLayer {
  ...
  private _getRandomRegisteryEntry(id, zeros = 6) {
    if (!this._randomRegistery.hasOwnProperty(id)) {
      let randomNumber = this._getRandomNumber(zeros);
    ...
    return new Promise((resolve, reject) => {
      if (Math.random() > 2) {
        ...
        setTimeout(() => {
          ...
          let object = Object.assign(test, { DocLines: meds });
          resolve(object);
        }, 1000);
      }
    });
  ...
  public produitDetail(id: string): Promise<any> {
    id = id.toUpperCase();
    return new Promise((resolve, reject) => {
      let response = products.find(item => {
        return item.Code === id;
      });
  ...
  public produitGenerique(DCI): Promise<Array<any>> {
    return new Promise((resolve, reject) => {
      let response = products.filter(item => item.DCI === DCI);
